...
doip_state_t doip_states [11] =
   {
	    ...
	    { Alive_Check_request                             ,NULL ,  Alive_Check_response                       ,NULL ,"Alive_Check_request"},
	    { DoIP_entity_status_request                      ,NULL ,  DoIP_entity_status_response                ,NULL ,"DoIP_entity_status_request"},
	    { Diagnostic_power_mode_information_request       ,NULL ,  Diagnostic_power_mode_information_response ,NULL ,"Diagnostic_power_mode_information_request"},
	    { Diagnostic_message                              ,NULL ,  Diagnostic_message_positive_acknowledgement,NULL ,"Diagnostic_message"},
    ...
    for (int i = 0x00; i < sizeof(doip_states)/sizeof(doip_state_t); i++)
    {
		if(htons(doip_msg.doip_header.type) == doip_states[i].request)
		{
			...
			if (0x00 == result)
			{
	            ...
	            soad_send_udp_msg(soad_handler, &doip_msg , 33 + sizeof(doip_header_t) , 13400);
			}
			break;
		}
	}
